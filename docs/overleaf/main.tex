...
Click in this order: the four \emph{fixed} frame dots
(\cmd{F1}--\cmd{F4}: lower-left, lower-right, upper-right, upper-left), then the
four \emph{moving} plate dots (\cmd{M1}--\cmd{M4}) in the same order. Use
\cmd{u}/Backspace to undo, \cmd{r} to restart, \cmd{q}/Escape to quit.

Clicks only need to be within about 14\,px; the tracker acquires the true
centroids on its first frame. Note that the fixed crosses sit \emph{outside} the
moving ones horizontally but \emph{inside} them vertically --- this looks wrong
and is correct.
