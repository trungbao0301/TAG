...
Why this exists
---------------
calib_results_tag.txt is an ocam model for a 1920x1200 capture, but the
...
Usage
-----
    ros2 run tag_state_estimation ai_map_estimator   # optional
...
from tag_state_estimation.core.ai_map_state import MarkerQuadGuard
...
PACKAGE_DIR = os.path.join(REPO_ROOT, "tag_state_estimation")


def warn_if_shadowed(logger):
    """Warn when another workspace shadows this repo on AMENT_PREFIX_PATH.

    ~/.bashrc sources ~/tag_ws, and install/setup.bash APPENDS, so this
    ...
    """
    try:
        resolved = get_package_share_directory("tag_state_estimation")
    ...
    if target(resolved) != target(PACKAGE_DIR):
        logger.warn(
            "ament resolves tag_state_estimation to %s, not %s. This "
...
class HoleCalibrator(Node):
    def __init__(self, args):
        super().__init__("tag_hole_calibrator")
        self.args = args
...
def main():
    ...
    parser.add_argument("--camera_topic", default="/tag_camera/image")
    parser.add_argument(
        "--frames_dir", default="",
