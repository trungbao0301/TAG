...
class _NumpyCvBridge:
    ...
    def imgmsg_to_cv2(self, msg):
        dtype, channels = self._ENCODINGS.get(msg.encoding, (np.uint8, 3))
        data = np.frombuffer(msg.data, dtype=dtype)
        row_items = msg.step // np.dtype(dtype).itemsize
        image = data.reshape(msg.height, row_items)
...
class TagGym(gym.Env):
    def __init__(
        ...
        self.client = self.node.create_client(
            HiwonderReset, "tag_hiwonder/reset"
        )

        self.br = CvBridge() if CvBridge is not None else _NumpyCvBridge()
