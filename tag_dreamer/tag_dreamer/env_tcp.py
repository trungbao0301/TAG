...
class TagGym(gym.Env):
    def __init__(
        ...
        self.ball_loss_grace_sec = max(
            0.0, float(os.environ.get("TAG_BALL_LOSS_GRACE_SEC", "0.35"))
        )
        self.occlusion_grace_sec = max(
            self.ball_loss_grace_sec,
            float(os.environ.get("TAG_OCCLUSION_GRACE_SEC", "1.50")),
        )
        ...
        self.max_angle_vel = float(os.environ.get("TAG_MAX_ANGLE_VEL", "240"))
        self.alpha_fac = float(os.environ.get("TAG_ALPHA_FAC", "-1.0"))
        self.beta_fac = float(os.environ.get("TAG_BETA_FAC", "-1.0"))
        self.max_cmd_1 = float(os.environ.get("TAG_MAX_CMD_1", "240"))
        self.max_cmd_2 = float(os.environ.get("TAG_MAX_CMD_2", "240"))
    ...
    def _active_ball_loss_grace(self):
        ...
        for start, end in self.occlusion_checkpoint_ranges:
            if start <= checkpoint <= end:
                return self.occlusion_grace_sec, checkpoint
        return self.ball_loss_grace_sec, checkpoint
