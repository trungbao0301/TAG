...
the board's tilt angles, and publishes on `/tag_state_estimation/*`.
...
```bash
ros2 topic hz /tag_state_estimation/estimate           # expect ~40 Hz
ros2 topic echo /tag_state_estimation/status --once    # expect: valid
```
...
Set `publish_legacy_topics:=false` to publish under `/tag_ai_map/*`
...
```bash
ros2 topic echo /tag_state_estimation/status
```
...
|---|---|---|
| `publish_legacy_topics` | `true` | false publishes under `/tag_ai_map/*` |
...
visualisation helpers, `ocam_model.py` and `calib/calib_results_tag.txt`.
