...
class CyberrunnerGym(gym.Env):
    def __init__(
        ...
        self.hole_warn_radius_m = float(
            os.environ.get("CYBERRUNNER_HOLE_WARN_RADIUS_M", "0.006")
        )
        self.near_hole_penalty = float(
            os.environ.get("CYBERRUNNER_NEAR_HOLE_PENALTY", "0.02")
        )
        self.checkpoint_reward = float(
            os.environ.get("CYBERRUNNER_CHECKPOINT_REWARD", "0.02")
        )
        self.path_stick_radius_m = float(
            os.environ.get("CYBERRUNNER_PATH_STICK_RADIUS_M", "0.015")
        )
        self.path_stick_reward = float(
            os.environ.get("CYBERRUNNER_PATH_STICK_REWARD", "0.001")
        )
        ...
        self.rest_duration_sec = float(
            os.environ.get("CYBERRUNNER_REST_DURATION_SEC", "180")
        )
