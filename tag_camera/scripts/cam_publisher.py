...
class CamPublisher(Node):
    def __init__(self, device):
        ...
        self.publisher = self.create_publisher(Image, "tag_camera/image", 1)
        self.br = CvBridge()
...
def main(args=None):
    args = sys.argv[1:]
    device = "/dev/video2" if not args else args[0]
    print("Using device: {}. To use a different device use the command-line argument, e.g.,\n"
          "ros2 run tag_camera cam_publisher.py /dev/video*".format(device))
