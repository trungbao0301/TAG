...
def main(argv=None):
    ...
    try:
        ...
        if args.script == "train":
            ...
            cleanup.append(env)
            agent = agt.Agent(env.obs_space, env.act_space, step, config)
            embodied.run.train_best(agent, env, replay, logger, args)
            ...
            cleanup.append(env)
            agent = agt.Agent(env.obs_space, env.act_space, step, config)
            embodied.run.train_top5(agent, env, replay, logger, args)
...
def make_replay(config, directory=None, is_eval=False, rate_limit=False, **kwargs):
    assert config.replay in ["uniform", "tag"] or not rate_limit
    length = config.batch_length
    size = config.replay_size // 10 if is_eval else config.replay_size
    if config.replay in ["uniform", "tag"] or is_eval:
