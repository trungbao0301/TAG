...
def train_best(agent, env, replay, logger, args):
    ...
    checkpoint_holder[0] = checkpoint
    ...
    def save_best_checkpoint(ep, worker):
        if checkpoint_holder[0] is None:
            return
        score = float(ep["reward"].astype(np.float64).sum())
        if score <= best_score[0]:
            return
        best_score[0] = score
        best_path = logdir / "checkpoint_best.ckpt"
        score_path = logdir / "checkpoint_best_score.txt"
        print(
            f"New best episode score {score:.3f}; "
            f"saving best checkpoint: {best_path}"
        )
        checkpoint_holder[0].save(best_path)
        score_path.write(
            f"score={score:.6f}\nstep={int(step)}\nepisode={int(episode)}\n",
            mode="w",
        )
    ...
    driver.on_episode(save_best_checkpoint)
